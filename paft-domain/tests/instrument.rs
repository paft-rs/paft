...
fn unique_key_distinguishes_asset_kind_for_symbol_identity() {
    ...
    assert_eq!(crypto.unique_key().as_ref(), "CRYPTO|SYMBOL|3:BTC");
    assert_eq!(equity.unique_key().as_ref(), "EQUITY|SYMBOL|3:BTC");
}
...
fn unique_key_distinguishes_asset_kind_for_symbol_exchange_identity() {
    ...
    assert_ne!(crypto.unique_key(), equity.unique_key());
    assert_eq!(
        crypto.unique_key().as_ref(),
        "CRYPTO|SYMBOL|3:BTC|EXCHANGE|NASDAQ"
    );
    assert_eq!(
        equity.unique_key().as_ref(),
        "EQUITY|SYMBOL|3:BTC|EXCHANGE|NASDAQ"
    );
}
...
fn unique_key_does_not_collapse_symbol_with_embedded_exchange_separator() {
    ...
    assert_eq!(
        symbol_only.unique_key().as_ref(),
        "CRYPTO|SYMBOL|10:BTC@NASDAQ"
    );
}
...
fn unique_key_namespaces_global_identifiers() {
    ...
    assert_eq!(with_figi.unique_key().as_ref(), "EQUITY|FIGI|BBG000B9XRY4");
    assert_eq!(with_isin.unique_key().as_ref(), "EQUITY|ISIN|US0378331005");
}
