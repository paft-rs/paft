...
const INCOME_STATEMENT_SCHEMA: &[(&str, DataType)] = &[
    ...
    (
        "net_income_common_stockholders.minor_units",
        DataType::UInt8,
    ),
    ...
    (
        "net_income_from_continuing_operations.minor_units",
        DataType::UInt8,
    ),
];
...
const CASHFLOW_SCHEMA: &[(&str, DataType)] = &[
    ...
    ("capital_expenditures.minor_units", DataType::UInt8),
    ("free_cash_flow.amount", DataType::Decimal(38, 10)),
    ("free_cash_flow.currency", DataType::String),
    ("free_cash_flow.minor_units", DataType::UInt8),
    ("net_income.amount", DataType::Decimal(38, 10)),
    ("net_income.currency", DataType::String),
    ("net_income.minor_units", DataType::UInt8),
    (
...
fn assert_money_values(df: &DataFrame, field: &str, expected: &[Option<i128>]) {
    ...
    let scales = df
        .column(&format!("{field}.minor_units"))
        .unwrap()
        .u8()
        .unwrap();
    ...
    for (index, mantissa) in expected.iter().enumerate() {
        ...
        assert_eq!(scales.get(index), mantissa.map(|_| 2));
    }
}
