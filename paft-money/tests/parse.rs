...
fn parse_plain_strings() {
    let m = Money::from_canonical_str("00123.4500", usd()).unwrap();
    assert_eq!(m.amount(), Decimal::from_str("123.45").unwrap());

    let negative = Money::from_canonical_str("-42.1", usd()).unwrap();
    assert_eq!(negative.amount(), Decimal::from_str("-42.1").unwrap());

    let positive = Money::from_canonical_str("+7.5", usd()).unwrap();
    assert_eq!(positive.amount(), Decimal::from_str("7.5").unwrap());

    let trimmed = Money::from_canonical_str(" 1.23 ", usd()).unwrap();
    assert_eq!(trimmed.amount(), Decimal::from_str("1.23").unwrap());
}

#[test]
fn parse_rejects_scientific_notation() {
    assert!(Money::from_canonical_str("1e3", usd()).is_err());
    assert!(Money::from_canonical_str("-2E-2", usd()).is_err());
}

#[test]
fn parse_rejects_invalid_tokens() {
    assert!(Money::from_canonical_str("abc", usd()).is_err());
