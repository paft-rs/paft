...
- Domain / Prediction: hand-rolled `Columnar` impls for `Instrument` and `PredictionInstrument` now build string columns via `StringChunked::from_iter_values` / `from_iter_options` over `&str` iterators. Previously they materialized a per-row `Vec<String>` (one heap allocation per row, per column) before letting Polars copy each `String` into the Arrow buffer; the new path writes straight into the Arrow string buffer, eliminating the per-row `String` allocation and the intermediate `Vec`. Visible behavior, schema, and cell values are unchanged. Note: the analogous pattern in `df-derive`-generated `Columnar` impls is still emitted by the macro — that lives in the `df-derive` repo and is tracked separately.
